...
export function PhaseFailed({
  ...
  const title = errorType === 'no-archetype-match'
    ? 'No matching application type'
    : errorType === 'repo-access-denied'
      ? 'Repository access denied'
      : 'Analysis failed';

  const message = errorMessage || 'The analysis engine encountered an error. Try again or contact your administrator.';
    ...
    <>
      ...
      {!errorType && (
        <InfoCard title="Error details">
          ...
          >
            <Typography variant="body2" color="error" style={{ fontFamily: 'inherit', fontSize: 'inherit' }}>
              ERROR: Analysis engine failed at step 3/4
            </Typography>
            <Typography variant="body2" color="textSecondary" style={{ fontFamily: 'inherit', fontSize: 'inherit' }}>
              {'  '}at RuleEngine.analyze(rules.java:142)
            </Typography>
            <Typography variant="body2" style={{ fontFamily: 'inherit', fontSize: 'inherit', color: 'inherit' }}>
      ...
      <Box mt={2} display="flex" alignItems="center" style={{ gap: 8 }}>
        <Button
          variant="contained"
          color="primary"
          onClick={() => store.executeAction(app.id, 'run-analysis', 'architect')}
        >
          Re-run analysis
        </Button>
